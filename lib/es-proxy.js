...
function proxyRequest(request, response, host, port, user, password, indexTrigger, getProxiedRequestPath, isUI) {
  var filteredHeaders = {};
  ...
  if (global.index_filter_usregex) {
    ...
    if ( (chkIndizes) && (chkIndizes.length > 0) ) {
      for ( var i = 0 ; i < chkIndizes.length ; i++ ) {
          if (chkIndizes[i].match(triggerRe)) {
            if (global.index_filter_usregex[request.user]) {
                var re=new RegExp(global.index_filter_usregex[request.user]);
...
exports.configureESProxy = function(app, esHost, esPort, esUser, esPassword, basePath, index_trigger) {
  app.use(basePath + "/__es", function(request, response, next) {
    proxyRequest(request, response, esHost, esPort, esUser, esPassword, index_trigger, 
